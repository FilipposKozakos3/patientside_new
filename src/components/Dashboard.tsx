...
export function Dashboard({
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // 2) Fetch counts from Supabase tables
      const [
        { count: medsDbCount },
        { count: allergiesDbCount },
        { count: labsDbCount },
      ] = await Promise.all([
        supabase
          .from("medications")
      ...
      let medCount = medsDbCount ?? 0;
      let allergyCount = allergiesDbCount ?? 0;
      let labCount = labsDbCount ?? 0;

      // Optional: count manual meds/allergies stored as health_records
      const [
        { count: manualMedCount },
        { count: manualAllergyCount },
      ] = await Promise.all([
        supabase
          .from("health_records")
          .select("id", { count: "exact", head: true })
      ...
      if (typeof manualMedCount === "number") {
        medCount += manualMedCount;
      }
      if (typeof manualAllergyCount === "number") {
        allergyCount += manualAllergyCount;
      }

      // 3) Recent activity from health_records
      ...
      if (dbHealthError) {
        console.error("Dashboard: error fetching health_records", dbHealthError);
      }

      const recent = (dbHealthRecords ?? []).map((row) => ({
        id: row.id,
      ...
      // 4) Stats object used by the Health Summary card
      setStats({
        byCategory: {
          medication: medCount,
      ...
      console.error("Dashboard: loadDashboardData failed", err);
      setRecentRecords([]);
      setStats({
        byCategory: {
          medication: 0,
